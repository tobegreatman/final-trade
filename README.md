...
1. **MACD**（权重 1.0）— 真实金叉/死叉检测（hist 穿越零轴）、零轴位置、柱状图变化、背离检测；动量衰减判 neutral 而非反转；弱信号降权 ×0.7
2. **涨跌家数**（权重 1.5）— 沪深京A股涨跌比 + 历史趋势
3. **RSI**（权重 1.0）— RSI(14) 超买超卖与趋势方向、背离检测；RSI≥60 高位偏多（×0.7）、<40 偏弱（×0.7）、超卖回升需 RSI>45
4. **融资余额**（权重 1.2）— 10 日线性回归斜率 + 加速度分析
5. **量价配合**（权重 1.3）— OBV 趋势（归一化斜率 ±0.05% 死区）+ 顶底背离检测 + 成交额/量智能显示；不显著时返回 neutral
6. **北向资金**（权重 1.5）— 5 日/20 日均量比 + 成交额趋势方向
7. **涨跌停**（权重 1.3）— 并行评分（涨跌比/跌停数/涨停质量/打板次日收益/封板率/赚钱效应）；跌停 0 家时按量级映射避免失真；跌停 ≥10 即扣分
